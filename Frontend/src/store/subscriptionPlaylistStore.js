...
export const UsePlayListStore = create((set, get) => ({
  ...
  createPlaylist: async (PlayListData) => {
    try {
      ...
      const res = await axiosInstance.post(
        "/subscription-Playlist/sub-create-playlist",
        PlayListData
      );
      set((state) => ({
  ...
  getAllPlayLists: async () => {
    try {
      set({ isLoading: true });
      const res = await axiosInstance.get("/subscription-Playlist/");
  ...
  getPlayListDetails: async (playlistId) => {
    try {
      set({ isLoading: true });
      const res = await axiosInstance.get(`/subscription-Playlist/${playlistId}`);
      set({ currentPlayList: res.data.playlist.problems });
    } catch (error) {
  ...
  addProblemToPlaylist: async (playlistId, problemIds) => {
    try {
      const res = await axiosInstance.post(
        `/subscription-Playlist/${playlistId}/add-problem`,
        {
          problemIds,
        }
      );
  ...
  removeProblemFromPlaylist: async (playlistId, problemIds) => {
    try {
      ...
      const res = await axiosInstance.delete(
        `/subscription-Playlist/${playlistId}/remove-problem`,
        {
          data : {problemIds},
        }
      );
  ...
  deletePlayList: async (playlistId) => {
    try {
      set({ isLoading: true });
      const res = await axiosInstance.delete(`/subscription-Playlist/${playlistId}`);
