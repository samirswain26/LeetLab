...
export const executeCode = async (req, res) => {
  try {
    ...
    // Store Submissions
    const submission = await db.SubscriptionSubmission.create({
      data: {
        userId,
        problemId,
    ...
    if (allPassed) {
      await db.SubscriptionproblemSolved.upsert({
    ...
    await db.SubscriptionTestCaseResult.createMany({
      data: testCaseResults,
    });

    const submissionWithTestCase = await db.SubscriptionSubmission.findUnique({
      where: {
        id: submission.id,
      },
      include: {
